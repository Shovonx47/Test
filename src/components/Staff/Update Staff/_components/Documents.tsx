...
interface DocumentInfoProps {
    ...
    singleStaff: DocumentProps;
}

const Documents = ({ control, setValue, singleStaff }: DocumentInfoProps) => {
    // Local states for updated file names and URLs
    const [resumeFile, setResumeFile] = useState<string>(singleStaff?.data?.resume || "");
    const [joiningLetterFile, setJoiningLetterFile] = useState<string>(singleStaff?.data?.joiningLetter || "");
    const [resumeUrl, setResumeUrl] = useState<string | null>(singleStaff?.data?.resume || null);
    const [joiningLetterUrl, setJoiningLetterUrl] = useState<string | null>(singleStaff?.data?.joiningLetter || null);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
                    ...
                    <div>
                        ...
                        <Controller
                            name="resume"
                            control={control}
                            defaultValue={singleStaff?.data?.resume || ""}
                            render={({ field }) => (
                    ...
                    <div>
                        ...
                        <Controller
                            name="joiningLetter"
                            control={control}
                            defaultValue={singleStaff?.data?.joiningLetter || ""}
                            render={({ field }) => (
                                <div className="mt-3 flex items-center gap-3">
